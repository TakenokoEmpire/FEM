...
# 基本設定
dt = 2e-7 # [s] ルンゲクッタ法なら、qty_x=10でdt=20e-7、14で10、20で5、44で1、100で0.2。自由端でも、同じ条件でいけそう。
fs = 0.01/dt # サンプリング周波数[Hz] ヤング率を補正するなら値を変更する（2e11で500k、2e5で5k？）
simu_time = 200e-3 # [s] 最小の固有振動数の波1つ分が7.5ms
qty_x = 20 # x方向の要素の数 33(両端自由の場合は32)以上だと一気に遅くなる。
...
# 解析条件設定
fix_type = "" # "left"で、左端（点0）固定の片持ち梁になる。
...
if fix_type == "left":
    ...
    lambda_const_list = [4.730, 7.853, 10.996] # 両端自由端
for lambda_const in lambda_const_list:
    ...
    print("固有振動数予測: {}".format(estimated_natural_freq))

# 減衰関係。
damping_ratio = 0.00 # 本当の減衰比ではない
...
# 治具関連
k_jig = [k_total * 10000, k_total * total_length * 40e-0]  # 治具の剛性　回転バネ（奇数番目）を設定すると、dt限界以下でも発散する（回転バネの剛性値が大きいほど発散しやすい。両端自由の場合、端部におくと発散しやすい。通常バネと回転バネが同じ位置にあると発散しやすい、同じ位置でなければ近いほうが若干発散しにくい？）
# 何番目の位置にジグを入れるか ※番号は0スタートなので注意
jig_position = [2 * int(qty_x * 0), 2 * int(qty_x * 0)+1] # 点0の変位、点0の角度、点1の変位、…のList。番号に注意


print("回転の固有振動数：{}".format((k_jig[1] / (m_total * total_length / 3) )**0.5))

# アニメーションの再生速度
animation_speed = int(fs / 20000) # int(fs / [アニメーションのHz])→指定Hzで再生可能
# ここまで入力
...
# エラー警告や情報表示
# 発散判定
print("dt限界: {:.2f} %".format(qty_x ** 2 * dt / 0.0002 * 100))
...
# 治具関連
for order, k in enumerate(k_jig):
    if k != 0:
        break
    print("治具{}が設定されていません".format(order))
...
for var_name in range(qty_x):
    for j1 in range(4):
        for j2 in range(4):
            m_mat_kari[var_name*2 + j1, var_name*2 + j2] += m_mat_element[j1, j2]

# 減衰マトリクスを作成
c_mat_kari = np.zeros((vec_length_kari, vec_length_kari))
# 角度（奇数番目）には減衰を設定せず、位置（偶数番目）に対してのみ減衰を設定する
for var_name in range(vec_length_kari):
    if var_name%2 == 0:
        c_mat_kari[var_name, var_name] += c_diag
...
for var_name in range(qty_x):
    for j1 in range(4):
        for j2 in range(4):
            k_mat_kari[var_name*2 + j1, var_name*2 + j2] += k_mat_element[j1, j2]
...
if fix_type == "left":
    ...
    for var_name in range(vec_length):
        for j in range(vec_length):
            m_mat[var_name, j] = m_mat_kari[var_name+2, j+2]
            c_mat[var_name, j] = c_mat_kari[var_name+2, j+2]
            k_mat[var_name, j] = k_mat_kari[var_name+2, j+2]
else:
    m_mat = m_mat_kari
    c_mat = c_mat_kari
    k_mat = k_mat_kari
...
for order,jig_pos in enumerate(jig_position):
    k_mat[jig_pos, jig_pos]  += k_jig[order]
...
for var_name in range(1000):
    f_vec = np.zeros((vec_length, 1))
    f_vec[f_position, 0] += f_wave_shape[0, var_name]
...
# 31行目から解析結果
# 0行目は加振力の時間軸波形
# 1行目以降が、変位の時間軸波形（n番目の点はn+1行目に保存される）
condition_box = []

var_list = ["var_list", "var_qty", "dt", "fs", "simu_time", "qty_x", "total_length", "height", "depth", "young", "density", "fix_type", "damping_ratio", "f_position", "f_wave_type", "f_mag", "f_freq", "k_jig", "jig_position", "animation_speed",  "sampling_length", "vec_length", ]

var_qty = len(var_list)

for var_name in var_list:
    condition_box.append([var_name, locals()[var_name]])

# condition_box.append(["dt",dt])
# condition_box.append(["fs",fs])
# condition_box.append(["simu_time",simu_time])
# condition_box.append(["qty_x",qty_x])
# condition_box.append(["total_length",total_length])
# condition_box.append(["height",height])
# condition_box.append(["depth",depth])
# condition_box.append(["young",young])
# condition_box.append(["density",density])
# condition_box.append(["fix_type",fix_type])
# condition_box.append(["damping_ratio",damping_ratio])
# condition_box.append(["f_position",f_position])
# condition_box.append(["f_wave_type",f_wave_type])
# condition_box.append(["f_mag",f_mag])
# condition_box.append(["f_freq",f_freq])
# condition_box.append(["k_jig",k_jig])
# condition_box.append(["jig_position",jig_position])
# condition_box.append(["animation_speed",animation_speed])

csv_box = np.insert(x_box, 0, f_wave_sampled, axis=1)

csv_write.csv_write_2d_list(csv_box, "", condition_box)
# "elements{}_measurement_pos{}_jig_pos{}_force_pos{}_dt{}_simu_time{}".format(qty_x, measurement_positions, jig_position, f_position, dt, simu_time)


# # 結果表示はここから（これ以降を分離したい）

# for var_name in range(len(measurement_positions)):
#     # plt.plot(yokojiku, signal_box[0])
#     plt.plot(yokojiku, x_box[:,measurement_positions[var_name]])
...
# x_box_even = []
# for var_name in range(int(len(x_box[1,:]) / 2)):
#     x_box_even.append(x_box[:, var_name*2])
