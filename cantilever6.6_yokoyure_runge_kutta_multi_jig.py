...
# 基本設定
dt = 5e-7 # [s] ルンゲクッタ法なら、qty_x=10でdt=20e-7、14で10、20で5、44で1、100で0.2。自由端でも、同じ条件でいけそう。
fs = 0.1/dt # サンプリング周波数[Hz] ヤング率を補正するなら値を変更する（2e11で500k、2e5で5k？）
simu_time = 30e-3 # [s] 最小の固有振動数の波1つ分が7.5ms
qty_x = 10 # x方向の要素の数 33以上だと一気に遅くなる。
...
# 減衰関係。
damping_ratio = 0.000 # 本当の減衰比ではない
...
# 治具関連
k_jig = [int(k_total * 10000), int(k_total * 10)]  # 治具の剛性　回転バネ（奇数番目）を設定すると、dt限界以下でも発散する（回転バネの剛性値が大きいほど発散しやすい。両端自由の場合、端部におくと発散しやすい。通常バネと回転バネが同じ位置にあると発散しやすい、同じ位置でなければ近いほうが若干発散しにくい？）
