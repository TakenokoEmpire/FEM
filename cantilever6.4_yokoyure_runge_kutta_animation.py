...
# 改善点
# 出力されるグラフを、FRFにする（加振力で割る、現状はただの変位のフーリエ変換）
# 治具を複数個おけるようにする
...
simu_time = 250e-3 # [s] 最小の固有振動数の波1つ分が7.5ms
...
# 力のFFT
# 問題点：dt<サンプリング間隔なので、力もサンプリング間隔でFFTしないと変位/力ができなくなる。
# 一方、インパルス加振はdt秒しか加振しないので、dt間隔でなければ正しく力がFFTできない。
# f_sampled = []
# for i in range(simu_length):
#     f.sampled.
# fft.main(f_wave_shape, 1/fs)
