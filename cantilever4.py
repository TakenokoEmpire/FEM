...
# 要求事項
# dtを小さくするなら、「計算周波数（dt）」だけでなく「サンプリング周波数」を設定しないと、蓄積データが大きくなりすぎて重くなる


# パラメータを設定
# 解析対象のパラメータ
total_length = 200e-3 # 全体の長さ[m]
height = 10e-3 # [m]
depth = 20e-3 # [m]
...
# 要素と壁の間、要素同士、治具、の剛性
k_internal = cross_section / element_length * young
k_wall = k_internal # 壁（固定端）と片持はりの接点の剛性
k_jig = 0
# 減衰関係。
damping_ratio = 0.05
c_diag = int(2 * (m_element * k_internal) ** 0.5 * damping_ratio) 
# 
dt = 1e-8 # [s] 10^-3だとちゃんとできない（無限増幅する）、サイン加振なら10^-5でおk（ダメかも？）、インパルス加振なら10^-7.5以上必要　→減衰を入れないとこうなるっぽい
fs = 50000 # サンプリング周波数[Hz] ヤング率を補正するなら値を変更する（2e11で500k、2e5で5k？）
simu_time = 0.01 # [s] ヤング率を補正するなら値を変更する（2e11で0.05、2e5なら1以上）
# 測定点の位置 list型で複数位置指定可能 ※番号は0スタートなので注意
measurement_positions = [0, 3]
...
# 減衰マトリクスを作成
c_mat = np.identity(qty_x) * c_diag
