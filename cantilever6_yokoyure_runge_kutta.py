...
fs = 1/dt # サンプリング周波数[Hz] ヤング率を補正するなら値を変更する（2e11で500k、2e5で5k？）
simu_time = 0.02 # [s] ヤング率を補正するなら値を変更する（2e11で0.05、2e5なら1以上）
...
x_box = runge_kutta_v2(m_mat, c_mat, k_mat, dt, simu_time, state_vec[:vec_length].ravel(), state_vec[-vec_length:].ravel())
