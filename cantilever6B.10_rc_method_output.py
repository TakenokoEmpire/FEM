...
# 解析を行う（アニメーション含む）か、RC法でRCを求めるか（全く別の目的）
analyze_type = "rc"

# 結果を表示したい点を選択
measurement_positions = [2,4] # あんま使ってない
fft_positions = [-2] # -2で右端

# 既定値よりも速くアニメーションを再生したい場合　int
animation_speed_boost = 100

# 解析対象のファイルの番号をList[int]で選択。RC解析の場合は、複数番号指定は動作未確認。
csv_file_mumber_list = [76]

csv_name = "{}_g11".format(csv_file_mumber_list[0])
...
def frf_general(input_multi, output_multi, dt, t_axis):
    ...
    https://watlab-blog.com/2020/07/05/frf/"""
    ...
    for csv_file_order in range(csv_file_qty):
        ...
        # FRFを関数を実行して計算
        h_io, mag, phase, freq = frf_core(input, output, 1 / dt)
        ...
        ax4.plot(freq, mag, lw=1, color = color_maps[csv_file_order])
...
for position_num in fft_positions:
    mag, phase, freq = frf_general(f_wave_sampled_box, x_box_box[:, :, position_num], 1/fs, t_sampled)
    
    if analyze_type == "rc":
        csv_box = []
        csv_box.append(mag)
        csv_box.append(phase)
        csv_box.append(freq)
        csv_box = np.array(csv_box)
        csv_write.csv_write_2d_list(csv_box, csv_name, [["analyze_target", csv_file_mumber_list[0]],["fft_position", position_num]], [], "null")
        exit()
